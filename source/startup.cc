...
static void _initialize()
{
    ...
    // Initialise UI translations (after DB, so dat/ paths are available).
    _loading_message("Loading translations...");
    init_ui_translations();
...
static void _construct_game_modes_menu(shared_ptr<OuterMenu>& container)
{
    for (size_t i = 0; i < entries.size(); ++i)
    {
        ...
        label->set_text(formatted_string(tr(entry.label), WHITE));
...
#endif
        btn->id = entry.id;
        btn->description = tr(entry.description);
...
{
    auto label = make_shared<Text>(formatted_string(tr("New Game"), WHITE));
...
public:
                  ...
                  selected_game_type(crawl_state.last_type)
    {
        ...
        auto name_prompt = make_shared<Text>(tr("Enter your name:"));
        ...
        auto mode_prompt = make_shared<Text>(tr("Choices:"));
        ...
        if (num_saves > 0)
        {
            auto save_prompt = make_shared<Text>(tr("Saved games:"));
