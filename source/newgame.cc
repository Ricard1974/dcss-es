...
static string _welcome(const newgame_def& ng)
{
    ...
    if (ng.species != SP_UNKNOWN)
        text = tr(species::name(ng.species));
    if (ng.job != JOB_UNKNOWN)
    {
        if (!text.empty())
            text += " ";
        text += tr(get_job_name(ng.job));
    }
    if (!ng.name.empty())
    {
        if (!text.empty())
...
{
    if (ng.species != SP_UNKNOWN)
        return tr(species::name(ng.species)) + "  ";
...
{
    string specs = chop_string(tr(species::name(ng.species)), 79, false);

    formatted_string prompt;
    prompt.cprintf(tr("You are a%s %s %s.").c_str(),
            (is_vowel(specs[0])) ? "n" : "", specs.c_str(),
            tr(get_job_name(ng.job)).c_str());
    ...
    vbox->add_child(make_shared<Text>(tr("Do you want to play this combination?")));
...
static void _choose_name(newgame_def& ng, newgame_def& choice)
{
    ...
    title.cprintf(tr("You are a%s %s %s.").c_str(),
            (is_vowel(specs[0])) ? "n" : "", specs.c_str(),
            tr(get_job_name(ng.job)).c_str());
    ...
    _add_menu_sub_item(sub_items, 0, 0,
            tr("Esc - Quit"), "", CK_ESCAPE, CK_ESCAPE);
    ...
    {
        auto tmp = make_shared<Text>();
        tmp->set_text(formatted_string(tr("Enter - Begin!"), BROWN));
        ...
        auto err = make_shared<Text>(
                formatted_string(tr("That's a silly name!"), LIGHTRED));
    ...
    while (!done && !crawl_state.seen_hups)
    {
        ...
        if (overwrite_prompt)
            prompt.cprintf("%s", tr("You have an existing game under this name; really overwrite?").c_str());
    ...
    const newgame_def& defaults)
{
    ...
    const string body_text = tr("Choose 0 for a random seed. "
    ...
    const string prompt_text = tr("Seed: ");
    ...
    pregen_check->set_child(make_shared<ui::Text>(tr("Fully pregenerate the dungeon")));
...
protected:
                                        ...
                                        const newgame_def& defaults)
    {
        ...
        if (choice_type == C_SPECIES)
            text = tr("+ - Recommended species");
        else
            text = tr("+ - Recommended background");
...
{
    ng_menu->_add_group_title(tr(name).c_str(), position);
                                   ...
                                   shared_ptr<OuterMenu>& sub_items)
{
    ...
    _add_menu_sub_item(sub_items, 0, 1, tr("% - List aptitudes"),
            "Lists the numerical skill train aptitudes for all races", '%',
            M_APTITUDES);
    _add_menu_sub_item(sub_items, 0, 2, tr("? - Help"),
                                         ...
                                         shared_ptr<OuterMenu>& sub_items)
{
    ...
    {
        _add_menu_sub_item(sub_items, 0, 0, tr("% - List aptitudes"),
                "Lists the numerical skill train aptitudes for all races",
                '%', M_APTITUDES);
        _add_menu_sub_item(sub_items, 0, 1, tr("? - Help"),
                "Opens the help screen", '?', M_HELP);
